...
def calculate_peripheral_size(peripheral, default_register_size):
	...
	for register in peripheral.registers:
		register_size = register._size if register._size is not None else default_register_size
...
if default_register_size is None:
	default_register_size = 32  # Default to 32-bit registers
...
for peripheral in peripherals:
	...
	for register in peripheral.registers:
		register_size = register._size if register._size is not None else default_register_size
		register_name = register.name if register.name else "REG_0x{:X}".format(register.address_offset)
		...
		print("\t\t\t{}({}:{})".format(register_name, hex(register.address_offset), hex(register.address_offset + register_size//8)))
		peripheral_struct.replaceAtOffset(register.address_offset, r_type, register_size//8, register_name, register.description)
