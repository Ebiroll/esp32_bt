...
struct co_list_hdr { // DWARF DIE: 1163
    struct co_list_hdr * next;
};

struct em_buf_node { // DWARF DIE: 6de
...
struct em_buf_tx_desc { // DWARF DIE: 161d
    uint16_t txptr;
    uint16_t txheader;
    uint16_t txdataptr;
    uint16_t txdle;
...
struct em_desc_node { // DWARF DIE: 15c1
    struct co_list_hdr hdr;
    uint16_t idx;
    uint16_t buffer_idx;
    uint16_t buffer_ptr;
...
struct co_list { // DWARF DIE: 1184
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct em_buf_env_tag { // DWARF DIE: 165f
...
struct llc_ch_asses_tag { // DWARF DIE: 309e
...
union anon_union.conflict1722_for___value { // DWARF DIE: 1722
    wint_t __wch;
    uchar __wchb[4];
};

struct _mbstate_t { // DWARF DIE: 1754
    int __count;
...
union anon_union.conflict1722 { // DWARF DIE: 1722
    wint_t __wch;
    uchar __wchb[4];
};
...
struct llm_test_mode { // DWARF DIE: 24b9
    _Bool end_of_tst;
...
struct data_len_ext { // DWARF DIE: 24e4
...
struct advertising_pdu_params { // DWARF DIE: 2344
...
struct t_public_key256 { // DWARF DIE: 257f
    uint8_t x[32];
    uint8_t y[32];
};
...
struct scanning_pdu_params { // DWARF DIE: 241b
...
struct ea_elt_tag { // DWARF DIE: 21f3
    struct co_list_hdr hdr;
...
struct le_chnl_map { // DWARF DIE: 3bb
    uint8_t map[5];
};

struct channel_map_assess { // DWARF DIE: 25b3
...
struct access_addr_gen { // DWARF DIE: 2484
    uint8_t intrand;
    uint8_t ct1_idx;
    uint8_t ct2_idx;
};

struct llm_le_env_tag { // DWARF DIE: 2624
...
struct llcp_con_param_req { // DWARF DIE: d4d
    uint8_t opcode;
    undefined field_0x1;
...
struct le_features { // DWARF DIE: 431
    uint8_t feats[8];
};

struct llcp_pause_enc_rsp { // DWARF DIE: c93
    uint8_t opcode;
};

struct rand_nb { // DWARF DIE: 415
    uint8_t nb[8];
};

struct llcp_ping_rsp { // DWARF DIE: f2d
    uint8_t opcode;
};

struct llcp_vers_ind { // DWARF DIE: cb0
    uint8_t opcode;
    uint8_t vers;
    uint16_t compid;
    uint16_t subvers;
...
struct llcp_feats_req { // DWARF DIE: c28
    uint8_t opcode;
    struct le_features feats;
};

struct llcp_start_enc_rsp { // DWARF DIE: be5
    uint8_t opcode;
};

struct llcp_unknown_rsp { // DWARF DIE: c00
    uint8_t opcode;
    uint8_t unk_type;
};

struct llcp_feats_rsp { // DWARF DIE: c50
    uint8_t opcode;
    struct le_features feats;
};

struct llcp_start_enc_req { // DWARF DIE: bca
    uint8_t opcode;
};

struct llcp_pause_enc_req { // DWARF DIE: c78
    uint8_t opcode;
};

struct llcp_reject_ind_ext { // DWARF DIE: ed7
    uint8_t opcode;
    uint8_t rej_opcode;
    uint8_t err_code;
};

struct llcp_length_req { // DWARF DIE: f4a
    uint8_t opcode;
    undefined field_0x1;
...
struct llcp_slave_feature_req { // DWARF DIE: d22
    uint8_t opcode;
    struct le_features feats;
};

struct llcp_reject_ind { // DWARF DIE: cf7
    uint8_t opcode;
    uint8_t err_code;
};

struct llcp_channel_map_ind { // DWARF DIE: ad9
    uint8_t opcode;
    struct le_chnl_map ch_map;
    uint16_t instant;
};

struct llcp_con_upd_ind { // DWARF DIE: a70
    uint8_t opcode;
    uint8_t win_size;
    uint16_t win_off;
    uint16_t interv;
...
struct llcp_terminate_ind { // DWARF DIE: b0e
    uint8_t opcode;
    uint8_t err_code;
};

struct llcp_con_param_rsp { // DWARF DIE: e12
    uint8_t opcode;
    undefined field_0x1;
    uint16_t interval_min;
    uint16_t interval_max;
...
struct init_vect { // DWARF DIE: 4f8
    uint8_t iv[4];
};

struct sess_k_div_x { // DWARF DIE: 4be
    uint8_t skdiv[8];
};

struct llcp_enc_req { // DWARF DIE: b36
    uint8_t opcode;
    struct rand_nb rand;
    uint8_t ediv[2];
...
struct llcp_length_rsp { // DWARF DIE: f9f
    uint8_t opcode;
    undefined field_0x1;
    uint16_t max_rx_octets;
    uint16_t max_rx_time;
...
struct llcp_enc_rsp { // DWARF DIE: b95
    uint8_t opcode;
    struct sess_k_div_x skds;
    struct init_vect ivs;
};

struct llcp_ping_req { // DWARF DIE: f10
    uint8_t opcode;
};

union llcp_pdu { // DWARF DIE: ff4
    uint8_t opcode;
...
struct llc_llcp_recv_ind { // DWARF DIE: 2a83
    uint8_t status;
    uint8_t dummy;
    union llcp_pdu pdu;
};
...
struct llc_data_ind { // DWARF DIE: 2a37
    uint16_t conhdl;
    uint8_t pb_bc_flag;
    undefined field_0x3;
    uint16_t length;
...
struct llc_con_upd_req_ind { // DWARF DIE: 2aee
    uint8_t operation;
    undefined field_0x1;
...
struct hci_le_set_data_len_cmd { // DWARF DIE: 76f
    uint16_t conhdl;
    uint16_t tx_octets;
    uint16_t tx_time;
};
...
struct ltk { // DWARF DIE: 3e8
    uint8_t ltk[16];
};

struct hci_le_start_enc_cmd { // DWARF DIE: 729
    uint16_t conhdl;
    struct rand_nb nb;
    uint16_t enc_div;
    struct ltk ltk;
...
struct __locale_t { // DWARF DIE: 1f1c
};
...
struct rwip_rf_api { // DWARF DIE: 1fee
    void (* reset)(void);
...
struct rwip_prio { // DWARF DIE: 2172
    uint8_t value;
    uint8_t increment;
};
...
struct ke_msg_handler { // DWARF DIE: 143b
    ke_msg_id_t id;
    undefined field_0x2;
    undefined field_0x3;
...
struct ke_state_handler { // DWARF DIE: 1467
...
struct sess_k_div { // DWARF DIE: 4db
    uint8_t skd[16];
};

typedef struct le_states le_states, *Ple_states;

struct le_states { // DWARF DIE: 49c
...
struct supp_cmds { // DWARF DIE: 453
    uint8_t cmds[64];
};
...
struct lld_conn { // DWARF DIE: 2d26
...
struct ea_interval_tag { // DWARF DIE: 22db
    struct co_list_hdr hdr;
...
struct lld_non_conn { // DWARF DIE: 2cd6
    uint32_t window;
    uint32_t anchor;
    uint32_t end_ts;
    _Bool initiate;
...
union lld_evt_info { // DWARF DIE: 2e15
    struct lld_non_conn non_conn;
    struct lld_conn conn;
};

struct lld_evt_anchor { // DWARF DIE: 2ca1
    uint32_t basetime_cnt;
    uint16_t finetime_cnt;
    uint16_t evt_cnt;
};

struct lld_evt_tag { // DWARF DIE: 2e3e
...
struct lld_evt_env_tag { // DWARF DIE: 2f39
...
struct _rand48 { // DWARF DIE: 1d12
    ushort _seed[3];
    ushort _mult[3];
    ushort _add;
};

struct _glue { // DWARF DIE: 1ccd
    struct _glue * _next;
    int _niobs;
    __FILE * _iobs;
};

struct anon_struct.conflict1ea0 { // DWARF DIE: 1ea0
    uchar * _nextf[30];
    uint _nmalloc[30];
};

struct __tm { // DWARF DIE: 1818
    int __tm_sec;
    int __tm_min;
    int __tm_hour;
    int __tm_mday;
...
struct anon_struct.conflict1d5b { // DWARF DIE: 1d5b
    uint _unused_rand;
    char * _strtok_last;
...
union anon_union.conflict1ee7_for__new { // DWARF DIE: 1ee7
    struct anon_struct.conflict1d5b _reent;
    struct anon_struct.conflict1ea0 _unused;
};

struct _on_exit_args { // DWARF DIE: 189b
    void * _fnargs[32];
    void * _dso_handle[32];
    __ULong _fntypes;
    __ULong _is_cxa;
...
struct __sbuf { // DWARF DIE: 1949
    uchar * _base;
    int _size;
};

struct __sFILE { // DWARF DIE: 1977
    uchar * _p;
    int _r;
    int _w;
    short _flags;
...
union anon_union.conflict1ee7 { // DWARF DIE: 1ee7
    struct anon_struct.conflict1d5b _reent;
    struct anon_struct.conflict1ea0 _unused;
};
...
struct llm_enc_ind { // DWARF DIE: 1517
    uint8_t status;
...
struct llm_enc_req { // DWARF DIE: 14ef
    struct ltk key;
...
struct rem_version { // DWARF DIE: 31c2
    uint8_t vers;
    undefined field_0x1;
    uint16_t compid;
    uint16_t subvers;
...
struct encrypt { // DWARF DIE: 31f7
    struct sess_k_div skd;
    struct ltk ltk;
    uint8_t randn[16];
};

struct data_len_ext_tag { // DWARF DIE: 322c
...
struct llc_env_tag { // DWARF DIE: 32bc
    void * operation[4];
    struct ea_elt_tag * elt;
...

{
  ...
  uVar2 = _L0();
  if ((uVar2 & 0xf) != 0xf) {
    uVar1 = CONCAT22(in_register_00002032,dest_id) >> 8;
    iVar5 = *(int *)(uVar1 * 4);
    if (((*(char *)(iVar5 + 0xa3) == '\x13') && (iVar3 = _LVL15(), iVar3 == 3)) ||
       ((*(char *)(iVar5 + 0xa4) == '\x0e' && (iVar3 = FUN_00010058(), iVar3 == 4)))) {
      (*pcRam00000000)(iVar5 + 0x91,param->encrypted_data,0x10,pcRam00000000);
    }
    else {
      iVar3 = FUN_00010068();
...

{
  ...
  uVar1 = _L0();
  if ((uVar1 & 0xf) != 0xf) {
    iVar4 = *(int *)((unaff_s1 >> 8) * 4);
    if (((*(char *)(iVar4 + 0xa3) == '\x13') && (iVar2 = _LVL15(), iVar2 == 3)) ||
       ((*(char *)(iVar4 + 0xa4) == '\x0e' && (iVar2 = FUN_00010058(), iVar2 == 4)))) {
      (*pcRam00000000)(iVar4 + 0x91,unaff_s2 + 1,0x10,pcRam00000000);
    }
    else {
      iVar2 = FUN_00010068();
      if (((iVar2 != 4) || ((uVar1 & 2) == 0)) || (*(char *)(iVar4 + 0xa4) != '\n')) {
        iVar2 = FUN_000100f8();
        if (iVar2 != 3) {
          return 0;
        }
        ...
        if (1 < (byte)(*(char *)(iVar4 + 0xa3) - 0xeU)) {
          return 0;
        }
      }
      puVar3 = (undefined2 *)((unaff_s1 >> 8) * 0x5c + 0x280080b4);
      uVar1 = 0xf;
      do {
        *puVar3 = CONCAT11(*(undefined *)(unaff_s2 + uVar1),((undefined *)(unaff_s2 + uVar1))[1]);
        uVar1 = uVar1 - 2 & 0xff;
        puVar3 = puVar3 + 1;
      } while (uVar1 != 0xff);
    }
...

{
  ...
  uVar1 = _L0();
  if (((uVar1 & 0xf) != 0xf) && ((uVar1 & 4) == 0)) {
    if ((uVar1 & 1) != 0) {
      return 2;
    }
    iVar3 = *(int *)((CONCAT22(in_register_00002032,dest_id) >> 8) * 4);
    _L0();
    iVar2 = (*pcRam00000000)(iVar3 + 99,5,pcRam00000000);
    if (iVar2 != 0) {
      FUN_000101c2();
      *(undefined *)(iVar3 + 0xa3) = 6;
      FUN_000101d2();
    }
  }
  return 0;
...

{
  ...
  if ((uVar1 & 0xf) != 0xf) {
    FUN_0001021e();
    FUN_00010232();
    puVar2 = (undefined2 *)FUN_00010248();
    *puVar2 = (short)(CONCAT22(in_register_00002032,dest_id) >> 8);
    _L0();
  }
  return 0;
}
...

{
  ...
  bVar1 = _L0();
  bVar1 = CONCAT31(extraout_var,bVar1) == 3;
  if (bVar1) {
    FUN_000103e6();
    *(undefined2 *)(unaff_s4 + 0x56) = *(undefined2 *)(unaff_s0 + 8);
    _L0();
...

{
  ...
  if ((uVar2 & 0xf) != 0xf) {
    ...
    iVar1 = *(int *)((CONCAT22(in_register_00002032,dest_id) >> 8) * 4);
    iVar3 = *(int *)(iVar1 + 0xc);
    if (iVar3 != 0) {
...

{
  ...
  do {
    _L0();
    uStack36 = extraout_a0_00;
    (*pcRam00000000)(iVar1,&uStack36,4,pcRam00000000);
    _L0();
    iVar2 = iVar1 + 0x10;
    iVar1 += 4;
    uStack36 = extraout_a0_01;
    (*pcRam00000000)(iVar2,&uStack36,4,pcRam00000000);
  } while (iVar1 != extraout_a0 + 0x10);
  FUN_000105e6();
  return;
}
...

{
  ...
  do {
    ...
    (*pcRam00000000)(iVar2,&stack0x0000000c,4,pcRam00000000);
  } while (iVar1 != extraout_a0 + 0x10);
  FUN_000105e6();
  return;
}
...

{
  ...
  uVar2 = CONCAT22(in_register_00002032,dest_id) >> 8;
  iVar5 = _LVL118();
  if (iVar5 == 3) {
    if ((uVar4 & 1) == 0) {
      return 0;
    }
    uVar4 = *(byte *)(*(int *)(uVar2 * 4) + 0xa3) - 10 & 0xff;
    if (9 < uVar4) {
      return 0;
    }
...

{
  ...
  iVar3 = *(int *)((CONCAT22(in_register_00002032,dest_id) >> 8) * 4);
  uVar1 = _L0(_DAT_0000002c);
  if ((uVar1 & 0xf) != 0xf) {
    if (*(char *)(iVar3 + 0xa3) != '\x06') {
      FUN_00010974();
      uVar1 = FUN_00010982();
      cVar2 = *(char *)(iVar3 + 0x3a) + -1;
      *(char *)(iVar3 + 0x3a) = cVar2;
      if ((cVar2 == '\0') || (uVar1 < 2)) {
        _LVL172();
      }
      (*pcRam00000000)(iVar3 + 99,auStack48,5,pcRam00000000);
      FUN_000109ca();
    }
    FUN_0001095a();
  }
...

{
  ...
  uVar1 = FUN_000109f8();
  if ((uVar1 & 0x7f) != 0x7f) {
    if ((*(byte *)(*(int *)(*(int *)((CONCAT22(in_register_00002032,dest_id) >> 8) * 4) + 0x10) +
                  0x7e) & 2) == 0) {
      FUN_00010a4e();
    }
    else {
...

{
  ...
  uVar1 = FUN_000109f8();
  if ((uVar1 & 0x7f) != 0x7f) {
    if ((*(byte *)(*(int *)(*(int *)((in_stack_0000000c >> 8) * 4) + 0x10) + 0x7e) & 2) == 0) {
      FUN_00010a4e();
    }
    else {
      FUN_00010a3c();
...

{
  ...
  if ((uVar1 & 0xf) != 0xf) {
    if ((uVar1 & 1) == 0) {
      ...
      if (iVar2 == 0) {
        FUN_00010ad2();
        *(undefined *)(*(int *)((unaff_s1 >> 8) * 4) + 0xa3) = 5;
        _L0();
        FUN_00010b06();
      }
    }
    else {
      if (*(char *)(*(int *)((unaff_s1 >> 8) * 4) + 0xa3) != '\x05') {
        return 2;
      }
    }
  }
...

{
  ...
  if ((uVar1 & 0xf) == 0xf) {
    ...
    if ((*(byte *)(*(int *)((CONCAT22(in_register_00002032,dest_id) >> 8) * 4) + 0xa5) & 8) == 0) {
      param[-1].conhdl = 0x806;
      *(short *)&param[-1].pb_bc_flag = (short)(CONCAT22(in_register_00002032,dest_id) >> 8);
      _L0();
      return 1;
    }
    FUN_00010b62();
...

{
  ...
  if ((uVar1 & 0xf) == 0xf) {
    ...
    if ((*(byte *)(*(int *)(unaff_s0 * 4) + 0xa5) & 8) == 0) {
      *(undefined2 *)(in_stack_0000000c + -8) = HCI_ACL_DATA_RX;
...

{
  ...
  if (iVar2 != 0x7f) {
    uVar1 = CONCAT22(in_register_00002032,dest_id) >> 8;
    puVar3 = (ushort *)(uVar1 * 0x5c + 0x28008092);
    *puVar3 = *puVar3 & 0xfcff;
    iVar2 = *(int *)(uVar1 * 4);
    uVar1 = _L0();
    if (((uVar1 & 1) != 0) && (uVar1 = *(byte *)(iVar2 + 0xa3) - 1 & 0xff, uVar1 < 0x13)) {
...

{
  ...
  iVar1 = _L0();
  if (iVar1 != 0x7f) {
    puVar4 = (ushort *)((unaff_s1 >> 8) * 0x5c + 0x28008092);
    *puVar4 = *puVar4 & 0xfcff;
    iVar1 = *(int *)((unaff_s1 >> 8) * 4);
    uVar2 = _L0();
    if (((uVar2 & 1) != 0) && (uVar2 = *(byte *)(iVar1 + 0xa3) - 1 & 0xff, uVar2 < 0x13)) {
