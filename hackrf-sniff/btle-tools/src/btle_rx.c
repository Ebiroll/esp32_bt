...
int parse_ll_pdu_payload_byte(uint8_t *payload_byte, int num_payload_byte,
                              LL_PDU_TYPE pdu_type, void *ll_pdu_payload) {
  int ctrl_pdu_type;
  ...
  if (pdu_type == LL_RESERVED || pdu_type == LL_DATA1 || pdu_type == LL_DATA2) {
    ...
    if (ctrl_pdu_type == LL_CONNECTION_UPDATE_REQ) {
      ...
      memcpy(ctrl_payload_type_R->payload_byte, payload_byte + 1,
             num_payload_byte - 1);
    }
  }
